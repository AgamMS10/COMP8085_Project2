...
def balance_data(data, target):
    ...
    min_samples = min(len(class_data[c]) for c in class_data)
    balanced_data = []
    for c in class_data:
        balanced_data.extend(class_data[c][:min_samples])
...
def split_json_file(input_file, output_train, output_test, output_val):
    ...
    balanced_records = balance_data(records, 'stars')

    
    processed_records = [process_record(record) for record in balanced_records]
    ...
    print(f"Total records (after removing 1-star reviews): {total_records}")
...
def main():
    ...
    split_json_file(
        "D:/COMP8085/Data/yelp_academic_dataset_review.json",
        'train1.json',
        'test1.json',
        'val1.json'
    )
